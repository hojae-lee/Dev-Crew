...
> "Read @docs/brief.md (first line contains PROJECT_NAME) and @docs/requirements.md. Design the system architecture and write the output to docs/architecture.md. Use the PROJECT_NAME from brief.md for all directory paths. Default to Next.js App Router fullstack — code lives in src/[PROJECT_NAME]/ (no frontend/backend split). If the user explicitly requests React+Vite, design React+Vite+TypeScript (FSD) frontend in src/[PROJECT_NAME]/frontend/ on port 5173 with Python+FastAPI backend in src/[PROJECT_NAME]/backend/ on port 8000 and SQLite at src/[PROJECT_NAME]/backend/app.db."
...
`docs/architecture.md`의 핵심 내용을 아래 형식으로 요약하여 사용자에게 보여준다:
